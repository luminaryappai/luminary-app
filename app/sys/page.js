...
const C={bg:"#191613",card:"#1E1B18",ink:"#F5F0E8",mid:"#A09282",dim:"#6B5D50",gold:"#BF8C3E",sage:"#7E9A6C",violet:"#8D80B8",border:"#2A2520"};
...
export default function Sys(){
  ...
  const tabs=["roster","detail","intel"];
  const ts=(t)=>({fontFamily:F.sans,fontSize:10,letterSpacing:2,textTransform:"uppercase",color:tab===t?C.gold:C.dim,background:tab===t?"rgba(191,140,62,0.08)":"transparent",border:"1px solid "+(tab===t?"rgba(191,140,62,0.19)":"transparent"),padding:"6px 14px",borderRadius:6,cursor:"pointer"});
  return(<div style={{minHeight:"100vh",background:C.bg,color:C.ink,fontFamily:F.sans,padding:16}}>
    ...
    {tab==="roster"&&<div>
      ...
      {users.map((u,i)=><div key={i} onClick={()=>{setSel(u);setTab("detail");}} style={{background:C.card,borderRadius:8,padding:12,marginBottom:6,cursor:"pointer",border:"1px solid "+C.border}}>
        ...
        <p style={{fontSize:10,color:C.dim,marginTop:2}}>{u.chart?.sun} {u.chart?.moon} {u.chart?.rising} · {u.chart?.intensity}/10{u.chart?.humanDesign?" · "+u.chart.humanDesign.type:""} · {new Date(u.createdAt).toLocaleDateString()}{u.chatHistory?.length?" · "+u.chatHistory.length+" chats":""}</p>
      </div>)}
    </div>}
    {tab==="detail"&&sel&&<div>
      ...
      {sel.friends&&sel.friends.length>0&&<p style={{fontSize:11,color:C.mid,marginTop:4}}>FRIENDS READ ({sel.friends.length}): {sel.friends.map(f=>f.name+" ["+f.mode+"]").join(" · ")}</p>}
      ...
      {sel.birthchartAnalysis&&<div style={{marginTop:12}}><p style={{fontSize:10,color:C.violet,letterSpacing:1}}>BIRTH CHART ANALYSIS:</p>
        {sel.birthchartAnalysis.headline&&<p style={{fontSize:11,color:C.gold,fontStyle:"italic",marginTop:4}}>{sel.birthchartAnalysis.headline}</p>}
        {sel.birthchartAnalysis.soulMantra&&<p style={{fontSize:11,color:C.mid,fontStyle:"italic",marginTop:4}}>Mantra: {sel.birthchartAnalysis.soulMantra}</p>}
      </div>}
